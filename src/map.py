...
from buildings import wall_level,cannon_level,wizard_tower_level,exploded_tiles
...
def printMap(V):
    ...
    for i in range(map.shape[0]):
        for j in range(map.shape[1]):
            wall_color = get_color(wall_level)
            if(map[i][j].split(':')[0] == pt.BLANK):
                for a in range(2):
                    for b in range(2):
                        map_matrix[2*i+a][2*j+b] = Back.GREEN + '  '
            elif(map[i][j].split(':')[0] == pt.WALL_TOP):
                map_matrix[2*i][2*j] = wall_color + '  '
                map_matrix[2*i][2*j+1] = wall_color + '  '
                map_matrix[2*i+1][2*j] = wall_color + '  '
                map_matrix[2*i+1][2*j+1] = wall_color + '  '
                ...
                if cannon_level == 1:
                    map_matrix[2*i][2*j+3] = color + ' 1'
                elif cannon_level == 2:
                    map_matrix[2*i][2*j+3] = color + ' 2'
                elif cannon_level == 3:
                    map_matrix[2*i][2*j+3] = color + ' 3'
                elif cannon_level == 4:
                    map_matrix[2*i][2*j+3] = color + ' 4'
                else:
                    map_matrix[2*i][2*j+3] = color + ' 5'
                ...
                if wizard_tower_level == 1:
                    map_matrix[2*i-1][2*j+2] = color + ' 1'
                elif wizard_tower_level == 2:
                    map_matrix[2*i-1][2*j+2] = color + ' 2'
                elif wizard_tower_level == 3:
                    map_matrix[2*i-1][2*j+2] = color + ' 3'
                elif wizard_tower_level == 4:
                    map_matrix[2*i-1][2*j+2] = color + ' 4'
                else:
                    map_matrix[2*i-1][2*j+2] = color + ' 5'
    ...
    for arch in stealth_archers:
        # if time.time() - arch.init_time > 10:
        a = 2*arch.position[0]
        b = 2*arch.position[1]
        map_matrix[a][b] = archColor(arch) + Fore.BLACK + 'ST'
        map_matrix[a+1][b] = archColor(arch) + Fore.BLACK + '  '
        map_matrix[a][b+1] = archColor(arch) + Fore.BLACK + 'AR'
        map_matrix[a+1][b+1] = archColor(arch) + Fore.BLACK + 'CH'
        # else:
        #     a = 2*arch.position[0]
        #     b = 2*arch.position[1]
        #     map_matrix[a][b] = Back.WHITE + Fore.BLACK + 'ST'
        #     map_matrix[a+1][b] = Back.WHITE + Fore.BLACK + '  '
        #     map_matrix[a][b+1] = Back.WHITE + Fore.BLACK + 'AR'
        #     map_matrix[a+1][b+1] = Back.WHITE + Fore.BLACK + 'CH'
    ...
    for bl in healers:
        ...
        map_matrix[a+1][b] = blColor(bl) + Fore.BLACK + 'ER'
        map_matrix[a][b+1] = blColor(bl) + Fore.BLACK + 'AL'
        map_matrix[a+1][b+1] = blColor(bl) + Fore.BLACK + '  '

    # for tile in exploded_tiles:
    #     xcoord = tile[0]
    #     ycoord = tile[1]
    #     xlist = [xcoord,xcoord-1,xcoord+1,xcoord-2,xcoord+2]
    #     ylist = [ycoord,ycoord-1,ycoord+1,ycoord-2,ycoord+2]
    #     for x in xlist:
    #         for y in ylist:
    #             map_matrix[x][y] = Back.WHITE

    # for i in range(map_matrix.shape[0]):
    #     for j in range(map_matrix.shape[1]):
    #         pair_indice = (i , j)
    #         if pair_indice in exploded_tiles:
    #             xidx = pair_indice[0]
    #             yidx = pair_indice[1]
    #             map_matrix[2*xidx][2*yidx] = Back.YELLOW + '  '
    #             map_matrix[2*xidx+1][2*yidx] = Back.YELLOW + '  '
    #             map_matrix[2*xidx][2*yidx+1] = Back.YELLOW + '  '
    #             map_matrix[2*xidx+1][2*yidx+1] = Back.YELLOW + '  '
